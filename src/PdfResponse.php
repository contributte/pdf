...
class PdfResponse implements Nette\Application\IResponse
{
	 ...
	 */
	public function setTemplate($source): PdfResponse
	{
		if (!($source instanceof Template) && !is_string($source)) {
			throw new InvalidArgumentException(
				sprintf(
					'Invalid source type. Expected (html) string or instance of
                     Nette\Bridges\ApplicationLatte\Template, but "%s" given.',
	...
	public function setDisplayZoom($displayZoom): void
	{
		if ($displayZoom <= 0 && !in_array($displayZoom, [
			...
			throw new InvalidArgumentException(sprintf("Invalid zoom '%s', use PdfResponse::ZOOM_* constants or o positive integer.", $displayZoom));
		}
	...
	public function setDisplayLayout(string $displayLayout): void
	{
		if ($displayLayout <= 0 && !in_array(
			...
			throw new InvalidArgumentException(sprintf('Invalid layout %s , use PdfResponse::LAYOUT* constants.', $displayLayout));
		}
	...
	public function setSaveMode(string $saveMode): void
	{
		if (!in_array($saveMode, [self::DOWNLOAD, self::INLINE], true)) {
			throw new InvalidArgumentException(sprintf("Invalid mode '%s', use PdfResponse::INLINE or PdfResponse::DOWNLOAD instead.", $saveMode));
		}
	...
	public function setBackgroundTemplate(string $pathToBackgroundTemplate): void
	{
		if (!file_exists($pathToBackgroundTemplate)) {
			throw new FileNotFoundException(sprintf("File '%s' not found.", $pathToBackgroundTemplate));
		}
